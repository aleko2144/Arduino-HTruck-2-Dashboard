...
int km_total;  //общий пробег автомобил€, кроме того, эта переменна€ использутс€ дл€ расчЄтов
float km_total_prev;
float km_local;  //текущий (суточный) пробег автомобил€, который можно сбросить
int km_total_nums[6]; //общий пробег, но кажда€ цифра по отдельности
int km_local_nums[5]; //текущий пробег, но кажда€ цифра по отдельности
...
int ValueToServo(float input_value, float value_max, float deg_max){
    ...
    //0.0 ... 1.0, насколько повЄрнута стрелка к макс. значению
...
void prepare_kilometrage(){
    ...
    km_local_nums[4] = (km_local - (int)km_local) * 10; //дес€тичный разр€д
